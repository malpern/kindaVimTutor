...
final class FinderDrillEngine {
    ...
    /// What the student has to accomplish per rep.
    enum Kind: Equatable {
        case navigate
        case duplicate
    }
    ...
    private(set) var kind: Kind = .navigate
    ...
    /// so a future drill can vary it per rep without touching the
    /// coaching panel.
    var currentTargetIconKey: String { FinderDrillPrototype.targetIconKey }
    ...
    private func startRep() {
        ...
        // For a duplicate drill, start the student directly on the
        // target — there's no navigation phase, they just need to
        // press yy + p. For a navigate drill, start at startIndex.
    ...
    private func selectionDidChange(to selection: String?) {
        ...
        case .duplicate:
            // Paste succeeded when Finder selects a file whose name
            // ends in " copy" (or similar) — a sibling that didn't
            // exist at drill start.
            if !originalFolderNames.contains(selection),
               !folderNames.contains(selection) {
                handleDuplicateCreated(name: selection)
            }
        }
    }

    /// Delete the newly-pasted duplicate and complete the rep.
    /// Running the real filesystem delete is fine because our tmp
    /// folder is disposable.
    private func handleDuplicateCreated(name: String) {
        if let folder {
